...
Target deployment is the always-on Mac Mini, not a public cloud product.

- Application: one web service supervised by the machine’s existing process manager.
...
Ports, process name, private hostname, database path, and backup destination will be recorded here when deployment is designed. No deployment occurs during the prototype phase.
