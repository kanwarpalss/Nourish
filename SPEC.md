...
| Nourish now runs on the Mac Mini, but not via the designed `launchd` + release/serve path | Open | Deployed 2026-08-23 as `pm2` process `nourish` running `npm run dev` (port 4317, `http://100.81.29.11:4317`), **not** `com.kanwar.nourish`/`npm run serve` as §10 specifies. Reason: `npm run serve` spawns `vinext start`, which runs the built output as plain Node — and `db/index.ts`'s top-level `import { env } from "cloudflare:workers"` throws `ERR_UNSUPPORTED_ESM_URL_SCHEME` there (that binding only resolves inside the Cloudflare Vite plugin's dev-time runtime). `npm run dev` works around it but loses the release-snapshot safety §10.1 exists specifically to provide (restart-after-failure, 10s throttle, dist/ isolation). No local `.wrangler/state` or diary data existed on the origin machine to carry over, so the Mac Mini's diary started fresh at `~/Library/Application Support/Nourish/nourish.db`. Fix properly by making the D1 import conditional/lazy so `vinext start` can run, then switch to the real `launchd` service. |
