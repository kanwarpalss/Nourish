...
3. **Every displayed nutrition value must carry its provenance tier**
   (Official label / Reference / Label mirror / Estimated). No unqualified
   numbers reach the screen.
4. **Never show a guessed macro as if it were known.** An uncertain cardIQ
...
```bash
npm test    # build + rendered-html + prototype-logic + cardiq-food
npm run lint
```
