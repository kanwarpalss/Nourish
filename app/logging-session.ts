...
export function isUserMealFood(food: Pick<NutritionItem, "id">) {
  return food.id.startsWith(USER_MEAL_PREFIX);
}
 ...
 * which is exactly why created foods must never reuse a seed id.
 */
...
export function upsertUserMeal(meals: UserMeal[], next: UserMeal): UserMeal[] {
  const withoutDuplicate = meals.filter((meal) => meal.id !== next.id);
  return [...withoutDuplicate, next].slice(-MAX_USER_MEALS);
}
 ...
 * scaled, so this re-scales through the same path a single food takes.
 */
export function userMealToFoods(meal: UserMeal): NutritionItem[] {
  return meal.components
    .filter((food) => isQuantityValid(food.unit, food.amount))
    .map((food) => scaleNutrition(food, food.amount));
}

/**
 * Renders a saved meal as a single row in the food list, so meals and foods can
 * share one result list without pretending a meal is a catalogue product.
 */
export function userMealAsListItem(meal: UserMeal) {
